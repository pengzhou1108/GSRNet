...
class DeepLabLFOVModel(object):
    ...
    def _create_network(self, input_batch, keep_prob, num_classes, use_fuse=False):
        ...
        for b_idx in range(len(dilations) - 1):
            for l_idx, dilation in enumerate(dilations[b_idx]):
                ...
                if b_idx<5:
                  conv = batchnorm(conv, scope='conv{:d}_batchnorm{:d}'.format(b_idx+1, l_idx+1))
        ...
        block_1 = tf.image.resize_bilinear(block_1,input_batch.get_shape()[1:3])
        block_2 = tf.image.resize_bilinear(block_2,input_batch.get_shape()[1:3])
        block_3 = tf.image.resize_bilinear(block_3,input_batch.get_shape()[1:3])

        current_final = tf.image.resize_bilinear(current,input_batch.get_shape()[1:3])
